@extends('main.layout.registration')
@section('content')
<!-- start: REGISTER BOX -->
    ...
    <form class="form-register" action="{{ URL::route('school-account-create-post'); }}" method="post">
        ...
        <fieldset>
            <div class="form-group">
                ...
                @if ($errors->has('name')) <p class="help-block alert-danger">{{ $errors->first('name') }}</p> @endif
            </div>
            <div class="form-group">
                <input type="text" class="form-control" name="manager_name" placeholder="Manager's Name" {{ (Input::old('manager_name')) ? 'value = "' .e(Input::old('manager_name')). '" ':'' }}>
